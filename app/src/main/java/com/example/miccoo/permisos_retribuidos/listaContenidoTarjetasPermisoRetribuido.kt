package com.example.miccoo.permisos_retribuidos

import com.example.miccoo.R

val listaContenidoTarjetasPermisoRetribuido: List<DataClassContenidoTarjetasPermisosRetribuidos> =
    listOf(
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.a_hospitalizacion,
            "A) Hospitalización, fallecimiento o intervención",
            "De 2 a 4 días",
            "Convenio:\n" +
                    "  a) Dos días laborales en casos de fallecimiento, accidente o enfermedad grave diagnosticada por facultativo o certificada por Institución Sanitaria, hospitalización o intervención quirúrgica que requiera reposo domiciliario de parientes hasta segundo grado de consanguinidad o afinidad. Cuando por tal motivo el trabajador necesite hacer desplazamientos al efecto, contados desde el lugar del centro de trabajo, el plazo será de cuatro días naturales. Si el desplazamiento es superior a 150 km en recorrido de ida y vuelta, el plazo será de cuatro días laborables."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.b_nacimiento,
            "B) Nacimiento",
            "3 o 4 días naturales",
            "Convenio:\n" +
                    "  b) Tres días laborales por nacimiento de hijos. Cuando por tal motivo el trabajador necesite hacer desplazamientos al efecto, contados desde el lugar del centro de trabajo, el plazo será de cuatro días naturales. Si el desplazamiento es superior a 150 km en recorrido de ida y vuelta, el plazo será de cuatro días laborables."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.c_matrimonio,
            "C) Matrimonio",
            "15 días naturales",
            "Convenio:\n" +
                    "  c) Quince días naturales por contraer matrimonio, pudiendo ser acumuladas al período de vacaciones, previo acuerdo con la Empresa."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.d_cumplimiento_deber,
            "D) Cumplimiento deber inexcusable",
            "El tiempo indispensable",
            "Convenio:\n" +
                    "  d) Por el tiempo indispensable para el cumplimiento de un deber inexcusable de carácter público y personal, comprendido el ejercicio del sufragio activo. Cuando conste en una norma legal o convencional un periodo determinado, se estará a lo que esta disponga en cuanto a duración de la ausencia y a su compensación económica."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.e_consulta_medica,
            "E) Consulta médica",
            "El tiempo indispensable",
            "Convenio:\n" +
                    "  e) Por el tiempo indispensable para asistir a consulta médica debidamente documentada por el médico, mediante justificante en que se acredite la hora de asistencia, cuando dicha asistencia coincida con el horario de trabajo."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.f_cambio_domicilio,
            "F) Cambio de domicilio",
            "1 día",
            "Convenio:\n" +
                    "  f) Un día por cambio de domicilio del trabajador o trabajadora."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.g_formacion,
            "G) Formación",
            "200 horas anuales",
            "Convenio:\n" +
                    "  g) Los trabajadores afectados por este Convenio podrán tener acceso a los permisos individuales de  formación durante un tiempo máximo de doscientas horas anuales, siempre que la asistencia a los mismos no distorsione el funcionamiento o necesidades de organización de la empresa, y siempre que exista acuerdo con la empresa."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.h_lactancia,
            "H) Lactancia",
            "15 días laborales",
            "Convenio:\n" +
                    "  h) Los trabajadores podrán hacer uso del permiso de lactancia en dos opciones: \n" +
                    "  a) Hacer uso de dicho permiso en los términos establecidos en la legislación general o\n" +
                    "  b) Acumular dicho permiso, en un único periodo de 15 días laborables a disfrutar por la mujer trabajadora, inmediatamente después de finalizar el periodo de ausencia  por maternidad. Esta opción podrá ser disfrutada indistintamente por el trabajador o la trabajadora, previo aporte del certificado administrativo que establezca que el padre o madre no hacen uso de dicho permiso."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.i_guarda_legal,
            "I) Guarda legal",
            "Reducción de jornada",
            "Convenio:\n" +
                    "  i) Reducción de jornada por guarda legal de menores de doce años o con discapacidad física, psíquica o sensorial, y para el cuidado directo de un familiar necesitado de cuidados especiales. Será de aplicación lo establecido en el apartado 6 del artículo 37 del Estatuto del Trabajador en los demás su puestos contemplados."
        ),
        DataClassContenidoTarjetasPermisosRetribuidos(
            R.drawable.j_asuntos_propios,
            "J) Asuntos propios",
            "3 días, 2 de ellos retribuidos",
            "Convenio:\n" +
                    "  j) Tres días retribuidos al año para asuntos propios sin justificar, de los cuales, dos de ellos se computarán como tiempo de trabajo efectivo sobre la jornada máxima anual de 1800 horas, respetándose las condiciones más beneficiosas de cada empresa o centro de trabajo, en todos los casos, solicitándolo con, al menos, noventa y seis horas de antelación, salvo acuerdo con la empresa, y que será obligatorio de conceder, salvo que coincidan en la petición del mismo día y en el mismo turno de trabajo más del 10% del personal perteneciente a una misma categoría de cada departamento o sección. Las peticiones serán atendidas por orden de solicitud, salvo causas de fuerza mayor acreditada. Los trabajadores podrán disfrutar de los días para asuntos propios por jornadas completas o por fracciones de cuatro horas, para cuidado o acompaña miento de ascendiente o descendientes a consulta  médica, debiendo respetarse, en todo caso, el sistema de solicitud y concesión previsto en el párrafo anterior.\n" +
                    "  Para aquellas empresas que su jornada de trabajo fuera inferior a la establecida en este convenio colectivo, dos días retribuidos de asuntos propios sin justificar, de los cuales, uno de ellos, se computará como de trabajo efectivo sobre la jornada máxima anual, y de una bolsa de ocho horas, para atender cuidados familiares en el año 2016 con justificación, respetando aquellos acuerdos que tengan más beneficiosos."
        ),
    )